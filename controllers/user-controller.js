...
module.exports = {
    ...
    getCartPage: async (req, res) => {
        const user = req.session.user
        let wcount = await userHelper.getWishlistCount(user._id)
        userHelper.getCartProducts(user._id).then((products) => {
            ...
            const count = 0
            res.render('user/cart', { user, error, count, wcount })
        })
    },
    ...
    orderPlaced: async (req, res) => {
        ...
        if (wallet) {
            userHelper.updateWallet(user._id, amount).then(() => {
                res.render('user/order-placed', { user, count, wcount })
            }).catch(() => {
                res.render('user/order-placed', { user, count, wcount })
            })
        } else {
    ...
    getSingleOrder: (req, res) => {
        ...
        try {
            userHelper.singleOrder(orderId).then((orderDetails) => {
                userHelper.getCartCount(user._id).then((count) => {
                    userHelper.getWishlistCount(user._id).then((wcount) => {
                        res.render('user/single-order', { user, orderDetails, count, wcount })
                    })
                })
            })
        } catch {
    ...
    postChangePassword: (req, res) => {
        const userId = req.session.user._id
        const data = req.body
        userHelper.changePassword(userId, data).then(() => {
            req.flash('passwordSuccess', 'Password updated successfully')
            res.redirect('/my-account')
        }).catch(error => {
            console.log(error)
            req.flash('passwordError', error.message)
            res.redirect('/my-account')
        })
    }
